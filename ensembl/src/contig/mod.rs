...
impl GRCh38Contig {
    ...
    pub fn ploidy(self, male: bool) -> u8 {
        match (self, male) {
            (Self::Y, true) => 1,
            (Self::Y, false) => 0,
    ...
    pub fn is_diploid(self, male: bool) -> bool {
        self.ploidy(male) == 2
    }
    pub fn is_haploid(self, male: bool) -> bool {
        self.ploidy(male) == 1
    }
...
// Note: intentionally hidden for now because of its weird ordering.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
struct GRCh37Contig {
    contig: &'static str,
}
...
mod tests {
    ...
    fn test_grch37_contig_chromosomes_sorted() {
        let mut chromosomes = GRCh37Contig::CHROMOSOMES.to_vec();
        chromosomes.sort();
        assert_eq!(&GRCh37Contig::CHROMOSOMES, &*chromosomes);
    }
