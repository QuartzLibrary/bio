...
impl ReadResource for PanUKBBS3Resource {
...
impl PhenotypeManifestEntry {
    ...
    pub fn load(resource: impl ReadResource) -> csv::Result<Vec<Self>> {
    ...
    pub async fn load_async(resource: impl ReadResource) -> csv::Result<Vec<Self>> {
        csv::ReaderBuilder::new()
            .delimiter(b'\t')
            .has_headers(true)
...
impl<Contig> SummaryStats<Contig> {
    pub fn load(resource: impl ReadResource) -> io::Result<impl Iterator<Item = csv::Result<Self>>>
    where
    ...
    pub fn at(&self) -> ContigPosition<Contig>
    where
        Contig: Clone,
    {
        ContigPosition {
